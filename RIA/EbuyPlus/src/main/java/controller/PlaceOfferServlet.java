	...
	protected void doPost(@NonNull final HttpServletRequest request, @NonNull final HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(userId == auction.getOwnerId()) {
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				response.getWriter().println("Offerer is also the owner");
				return;
			}
			...
			if(auction.isClosed()) {
				...
				response.getWriter().println("Can't place an offer on a closed auction");
				return;
			}
			...
			if(auction.isExpired()) {
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				response.getWriter().println("Can't place an offer on an expired auction");
				return;
			}
			...
			response.getWriter().println("Offer placed successfully");
