...
public class CheckLoginServlet extends HttpServlet {
	...
	protected void doPost(@NonNull final HttpServletRequest request, @NonNull final HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			final String showHomePath = getServletContext().getContextPath() + Values.Web.SHOW_HOME_SERVLET;
			response.sendRedirect(showHomePath);

		} catch (final InvalidCredentialsException e) {
			// user not found: redirect to login page
			final String showLoginPath = getServletContext().getContextPath() + Values.Web.SHOW_LOGIN_SERVLET;
			
			// display error message to the user
			request.getSession().setAttribute(Values.Params.ERROR_MESSAGE, "Invalid username or password");
			...
			e.printStackTrace();
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing or invalid credentials");
		}
	}
