...
public class CloseAuctionServlet extends HttpServlet {
	...
	protected void doPost(@NonNull final HttpServletRequest request, @NonNull final HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (auction.isClosed()) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Attempted to close an already closed Auction");
				return;
			}
			
			// check that auction has expired (can't close a non-expired auction)
			if(auction.getExpireDateTime().isAfter(LocalDateTime.now())) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Attempted to close a non-expired Auction");
				return;
			}
			...
			request.getSession().setAttribute(Values.Params.SUCCESS_MESSAGE, "Auction closed successfully");

			// redirect to auction details page
			final String showAuctionDetailsPath = getServletContext().getContextPath() + Values.Web.SHOW_AUCTION_DETAILS_SERVLET + "?" + Values.Params.AUCTION_ID + "=" + auctionId;
			response.sendRedirect(showAuctionDetailsPath);
			...
			e.printStackTrace();
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Unable to close Auction with the given ID");
		}
	}
